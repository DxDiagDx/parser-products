...
3. Активируйте его и установите зависимости: 
   ...
   ```
   Для VSCode и других IDE (кроме PyCharm)
   ```bash
   pip install -e .
   ```
   PyCharm сам предложит установить вируальное окружение и пропишет путь к модулям.
4. Скопируйте `template/config.example.py` в `config.py` и настройте под себя (опицонально)
